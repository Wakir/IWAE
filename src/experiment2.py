...
if __name__ == '__main__':
    ...
    metrics = [f1_score, g_mean, bac, precision_score, recall_score, specificity]
        ...
        Parallel(n_jobs=-1)(
            ...
            ({'streams_random_seeds': [random_seeds[idx]],
                ...
                'file': os.path.join(path,
                                    f'Stream_{str(names[q])}_imbalance_{ref.__name__}.npy')}
                for q in range(len(streams)) for ref in reference_methods))
    """Parallel(n_jobs=-1)(
        delayed(conduct_experiment)(**kwargs)
        for kwargs in
        ({'streams_random_seeds': [random_seeds[idx]],
            'ensembles': (new_WAE(base_estimator=base_classifier, scale=scale, n_classifiers=n, base_quality_measure=quality_measure, pruning_criterion=quality_measure, n_estimators= n_est),),
            'ensembles_labels': (f"new_wae_{str(scale)}_{str(n)}_cl",),
            'metrics': metrics,
            'stream': streams[q],
            'file': os.path.join(path,
                                f'Stream_{str(names[q])}_imbalance_new_wae_{str(scale)}_{str(n)}_cl.npy')}
            for q in range(len(streams)))
    )"""
else:
    if args.reference:
